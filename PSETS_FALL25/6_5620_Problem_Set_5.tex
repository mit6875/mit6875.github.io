...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        \begin{align*}
            \Pr_{k \gets \Gen(1^\lambda)}[h_{k}(x) = h_k(x') \text{ and } x \neq x': (x, x') =\calA(1^{\secp}, k) ] \leq \negl(\lambda).
        \end{align*}
    \end{enumerate}
\end{definition}
...
\begin{enumerate}
    ...
    \begin{enumerate}
        \item \textbf{Correctness of local opening:} For all $\secp \in \bbN$,  $k\in\Gen(1^\secp)$, $m \in \{0, 1\}^{\secp \cdot p}$, and $i \in [p]$, 
        ...
        \item \textbf{Computational binding of local opening:} For all poly-size adversaries $\calA$, all indices $i \in [p]$, and all $\secp \in \bbN$,
            \[ \Pr_{k \gets \Gen(1^\secp)}[\forall b \in \{0, 1\}, \Ver(k,\hash, L_b, i, m_b) = 1:~(\hash, L_0, m_0, L_1, m_1) = \calA(1^\secp, k)] \leq \negl(\secp). \]
    \end{enumerate}
\end{enumerate}
...
\begin{enumerate}
    ...
    \item \textbf{A Majority of Majorities (15 points).} Let $k(n)<n$ be any (polynomial-time computable) function of $n$, and suppose that the set $[n]$ is partitioned into $k$ sets $S_1, \hdots, S_k$ of size $\frac n k$. Suppose that we want a subset of players $S\subset [n]$ to be able to reconstruct the secret if and only if a \emph{majority} of the sets $S_1, \hdots, S_k$ have \emph{a majority} of players present in $S$. The corresponding access structure can be described as
...
\begin{definition}
    ...
    \begin{enumerate}
        ...
        %Moreover, given a witness $w$ for an instance $x \in \calL$, $P(\crs, x, w)$ runs in polynomial time.  
        ...
        \item \textbf{(Non-Adaptive, Single/Multi-Theorem) Zero-Knowledge:} If $(\Gen, P, V)$ is single-theorem zero-knowledge, then for every PPT $V^{*}$, there exists a PPT simulator $\calS$, such that for every polynomial time instance generator algorithm $\calI$, that on input $1^{\secp}$ outputs $(x, w) \in \calR_{\calL}$,
            \[ (\crs, x, P(\crs, x, w)) \approx S(1^{\secp}, x), \]
        where $\crs \gets \Gen(1^{\secp})$ and $(x, w) \gets \calI(1^{\secp})$.

        If $(\Gen, P, V)$ is multi-theorem zero-knowledge, For every PPT $V^{*}$ there exists a PPT simulator $\calS$, such that for every polynomial $p = p(\secp)$, every polynomial-time instance generator algorithm $\calI$, that on input $1^{\secp}$ outputs $(x_1, w_1), \ldots, (x_p, w_p) \in \calR_{\calL}$,
...
\begin{itemize}
    \item $\Setup(1^\secp, d) \to (\pk, \sk)$: takes as input the security parameter and a polynomial degree bound $d$ and outputs a private-key secret-key pair.
...
\begin{itemize}
    \item \textbf{Correctness}: For any $\pk \gets \Setup(^\secp, t)$, polynomial $f \in \F_p$ with degree at most $d$, for any $\com \gets \Commit(\pk, f)$, for any $u \in \F_p$, and $(v, \pi) \gets \Open(\pk, f, u)$, it must hold that $v = f(u)$ and $\Verify(\pk, \com,\allowbreak u, v, \pi) = \top$ with probability $1$. 
    \item \textbf{Binding}: For all adversaries $\calA$, there is a negligible function $\negl$ such that
    \begin{align*}
...
\end{remark}
We will now see a construction of such a commitment scheme. First, we introduce some additional structure in the groups that we will be using. Let $G$ be groups of prime order $p$ with generator $g$. Then group $G$ is called \textit{bilinear} if it is also equipped with an efficiently computable (symmetric) bilinear operation $e: G \times G \to G_T$ that maps pairs $(g_1, g_2) \in G \times G$ to an element of some other group $G_T$ of the same prime order $p$, such that the following properties hold.\footnote{Technically, all of these groups and orders are indexed by the security parameter $\secp$, but we have omitted them from the notation to avoid clutter.}
\begin{itemize}
    \item \textbf{Bilinearity}: For every $x, y \in \bbZ_p$, it holds that $e(g^x, g^y) = e(g, g)^{xy}$.
    \item \textbf{Non-degenerate}: For generator $g \in F$, we have that $e(g, g)$ generates $G_T$.
\end{itemize}

\paragraph{Construction.} Let $e : G \times G \to G_T$ be an efficiently computable bilinear map for groups $G, G_T$ of prime order $p$. 
% Assume that the discrete log problem is hard for the groups $G, G_T$.
\begin{itemize}
    ...
    \begin{align*}
        \mathsf{com} :=  \prod_{i = 0}^t g_i^{f_i}  = g^{f(\alpha)}.
    \end{align*}
    \item $\Open(\pk, f, u)$: Parse $\pk = (g_0, \ldots, g_t)$, and define the polynomial $f'$ and compute $\pi$ as follows:
    \begin{align*}
...
\textbf{Prove that this scheme satisfies correctness and binding, assuming the following strengthening of the Diffie Hellman assumption holds over $G$.}

\begin{definition}[$t$-strong Diffie Hellman Assumption]
    Let $\alpha \gets \bbZ_p^*$ be a random element. Let $g$ be a generator of the group $G$. For every poly-size adversary $\calA$, there is a negligible function $\negl$ such that, given as input the tuple $(g, g^\alpha, g^{\alpha^2}, \ldots, g^{\alpha^t}) \in G^{t+1}$, for any $c \in \bbZ_p \setminus \{-\alpha\}$, it holds that
    \begin{align*}
        \Pr_{\alpha \gets \bbZ_p^*}[\calA(g, g^\alpha, g^{\alpha^2}, \ldots, g^{\alpha^t}) \to (c, g^{(\alpha + c)^{-1}})] \le \negl(\secp).
    \end{align*}
\end{definition}
\end{document}