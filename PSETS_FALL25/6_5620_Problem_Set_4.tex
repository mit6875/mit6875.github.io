...
\end{definition}
Suppose there exists a re-randomizable secret key-encryption scheme $(\Gen, \Enc, \Dec, \RR)$. Show that there exists a (CPA-secure) public-key encryption scheme $(\Gen', \Enc', \Dec')$.

\subsection{PKE from additive homomorphism (15 points)}
Let $\SKE = (\Gen, \Enc, \Dec, \Add)$ be a \emph{secret-key} encryption scheme for single-bit messages that satisfies the standard compactness and security guarantees, and supports homomorphic addition:
\begin{itemize}
