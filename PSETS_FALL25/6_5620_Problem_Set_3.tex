...
\begin{enumerate}[resume]
    \item (5 points) Consider the field $\bbF := \bbF_{2^{\secp+1}}$.\footnote{If you want more information about finite fields, see references \href{https://www.cs.cmu.edu/~venkatg/teaching/codingtheory/notes/algebra-brief-notes.pdf}{here}, \href{https://ocw.mit.edu/courses/6-451-principles-of-digital-communication-ii-spring-2005/8921c013c1d7802bb197572c52442b97_chap7.pdf}{here}, and \href{https://people.math.harvard.edu/~landesman/assets/finite-fields.pdf}{here}.} If we interpret an input in $\{0, 1\}^{\secp+1}$ as an element of $\bbF$ (and vice versa)\footnote{We assume that the mapping associates the all zeros string $0^{\secp+1}$ with the field element 0.}, we can define the family of hash functions
...
\textcolor{red}{Note: A previous version of this question had a typo and incorrectly defined the type of $H_k$ as $H_k: \{\{0,1\}^m \}^* \to \{0,1\}^n$. However in this part, we want the hash function to take as input strings of arbitrary length.}
...
To make encryption efficient, sometimes the same small public exponent $e$ is used (e.g. $e = 3$) across multiple users. Now suppose a sender transmits encryptions with the same randomness $r$ to $k$ recipients whose public keys are $(N_1, e), \dots, (N_k, e)$, where each $N_i = p_i q_i$ is pairwise coprime. 
