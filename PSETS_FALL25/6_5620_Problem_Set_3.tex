...
\begin{definition}
	A hash family $(\Gen, \sfH = \{\sfH_{\hk} : \{0, 1\}^{n(\secp)} \to \{0, 1\}^{\secp}\}_{\hk \in \calK_{\secp}})$ is a UOWHF family if (a) $\secp < n(\secp)$ and (b) for all $x \in \{0, 1\}^n$ and all poly-size adversaries $\calA$,
...
\begin{enumerate}[resume]
    ...
    Prove that $\overline{\sfH}$ is a family of hash functions that compress their input by a single input and are always 2-to-1.
...
A \emph{compressing collision-resistant function family} is a collection of functions $\calF = \{\calF_\lambda\}_{\lambda \in \mathbb{N}}$ where $\calF_\lambda = \{f_{k}\}_{k \in \{0,1\}^{\lambda}}$ together with an efficient key generation algorithm $\Gen(1^\lambda) \to k$, such that the following two properties hold:
\begin{enumerate}
    ...
    \end{align*}
\end{enumerate}
Our goal is build a \textit{variable-length} collision-resistant hash function family $\calH = \{\calH_\lambda\}_{\lambda \in \mathbb{N}}$ where $\calH_\lambda = \{H_k \}_{k \in \{0,1\}^\lambda}$
\[
H_{k} : \{0,1\}^* \to \{0,1\}^n,
\]
...
\subsection{A compressing collision-resistant function (10 points)}
...
Assuming that the SIS assumption holds, give a family of collision-resistant compressing functions $\calF = \{\calF_\lambda\}_{\lambda \in \mathbb{N}}$ where $\calF_\lambda = \{f_k: \{0,1\}^M \rightarrow \mathbb{Z}_q^N\}_{k \in \{0,1\}^\lambda}$, where $M, N = \poly(\lambda)$.

\subsection{Extending to more input lengths (5 points)}
Suppose there exists a collision-resistant compressing function family $\calF = \{\calF_\lambda\}_{\lambda \in \mathbb{N}}$ where $\calF_\lambda = \{f_k\}_{k \in \{0,1\}^\lambda}$ and
$f_{k} : \{0,1\}^{n + m} \to \{0,1\}^n$. Define a hash function family $\calH' = \{\calH'_{\lambda}\}_{\lambda \in \mathbb{N}}$ where $\calH'_{\lambda} = \{H'_k\}_{k \in \{0,1\}^\lambda}$ and 
$$H'_{k}: \{\{0,1\}^{m}\}^* \to \{0,1\}^n.$$ 
...
\end{itemize}
Prove that this hash function family is ``collision-resistant'' in the following sense: no polynomial-size adversary is able to produce $x \neq x'$ such that $|x| = |x'| \in m \mathbb{Z}$ such that $H'_{k}(x) = H'_{k}(x')$.

\subsection{Insecure padding (5 points)}\label{sec:insecure-padding}
Now define the hash function family $\calH'' = \{\calH''_\lambda\}_{\lambda \in \mathbb{N}}$ where $\calH''_\lambda = \{H''_k \}_{k \in \{0,1\}^\lambda}$
\[
H''_{k} : \{0,1\}^* \to \{0,1\}^n,
\]
...
\end{align*}
where $\ell \in \{0,1, \ldots, m-1\}$ such that $x\|0^{\ell}$ has length that is a multiple of $m$. Give an attack on this hash function---that is, give an efficient algorithm that produces two inputs $x, x'$ such that $H''_k(x) = H_k''(x')$.

\subsection{Collision-resistant hash functions, finally (10 points)}
Suppose there exists a collision-resistant compressing function family $\calF = \{\calF_\lambda\}_{\lambda \in \mathbb{N}}$ where $\calF_\lambda = \{f_k\}_{k \in \{0,1\}^\lambda}$ and
$f_{k} : \{0,1\}^{n + m} \to \{0,1\}^n$. Define a hash function family $\calH = \{\calH_{\lambda}\}_{\lambda \in \mathbb{N}}$ where $\calH_{\lambda} = \{H_k\}_{k \in \{0,1\}^\lambda}$, 
$$H_{k}: \{\{0,1\}^{m}\}^* \to \{0,1\}^n$$
...
\begin{itemize}
    ...
    \[ \{ \pk: (\pk, \sk) \gets \Gen(1^{\secp}, \real) \} \approx \{ \widetilde{\pk}: \widetilde{\pk} \gets \Gen(1^{\secp}, \lossy)\}. \]
...
Show that every lossy encryption scheme (when operating in the real mode) is also semantically secure: that is,
    \[ \{ (\pk, \Enc(\pk, 0)): (\pk, \sk) \gets \Gen(1^{\secp}, \real) \} \approx \{ (\pk, \Enc(\pk, 1)): (\pk, \sk) \gets \Gen(1^{\secp}, \real) \}. \]
\subsection{DDH tuples (5 points)}
We will now see how to construct a lossy encryption scheme assuming the Decisional Diffie-Hellman assumption (DDH). Consider a cyclic group $\bbG$ of order $p$ on which DDH assumption holds. That is, suppose that for any generator $g \in \bbG$,
...
Modify the following encryption scheme by designing a lossy key generation algorithm and proving that the resulting scheme satisfies all three properties of a lossy encryption scheme (assuming DDH).
